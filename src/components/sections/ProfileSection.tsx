...
const ProfileSection: React.FC<ProfileSectionProps> = ({ onLogout, onOpenMessages }) => {
  ...
  const interests = profile?.interests || ['Adventure', 'Food', 'Culture'];
        ...
        <div className="mt-20 grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="travel-card p-6 text-center"><p className="text-3xl font-bold text-foreground">0</p><p className="text-muted-foreground">Trips</p></div>
              ...
              <div className="relative" ref={shareRef}>
                ...
                {showShareMenu && (
                  <div className="absolute bottom-full left-0 right-0 mb-2 bg-background border border-border rounded-2xl shadow-xl z-50 animate-fade-in overflow-hidden p-2">
                    ...
                    <button onClick={() => handleShareProfile('twitter')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm hover:bg-muted transition-colors"><Twitter className="w-4 h-4 text-sky-500" />Twitter / X</button>
                    <button onClick={() => handleShareProfile('facebook')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm hover:bg-muted transition-colors"><Facebook className="w-4 h-4 text-blue-600" />Facebook</button>
